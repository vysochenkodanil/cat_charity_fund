...
#     DonationDB,
# )
# from app.services.investment import invest_to_projects

# router = APIRouter()


# @router.post("/", response_model=DonationUserDB, response_model_exclude_none=True)
...
#     await invest_to_projects(new_donation, session)
#     return new_donation
...
#     session: AsyncSession = Depends(get_async_session),
# ):
#     """Посмотреть список своих пожертвований (только авторизованные пользователи)."""
#     return await donation_crud.get_by_user(user.id, session)


# @router.get(
#     "/", response_model=list[DonationDB], dependencies=[Depends(current_superuser)]
# )
...
from app.core.user import current_user, current_superuser
from app.crud.donation import donation_crud
from app.schemas.donation import DonationCreate, DonationUserDB, DonationDB
